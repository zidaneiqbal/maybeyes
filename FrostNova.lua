...
local function executeSkill(slot)
    ...
    local target
    if settings.SkillType == 1 then
        target = char
    else
        -- Target Mob path (Auto-Detect Closest)
        ...
        if closestMob then
            target = closestMob
            print("DEBUG: Found Closest Target: " .. target.Name .. " (Dist: " .. math.floor(minDist) .. ")")
        end
    end

    if not target and settings.SkillType == 2 then
        warn("DEBUG: No valid Mob Target found! Check if mobs are spawned.")
        return
    end
...
local function waitCooldownGUI(slotIndex)
    ...
    pcall(function()
        -- Menyesuaikan slotIndex (0-3) ke Nama (技能2, 3, 4) kalau slotIndex = 3 maka balikkan slotIndex = 2
        local pathName = "技能" .. (slotIndex + 1)
        if slotIndex == 4 then pathName = "技能2" end
        -- Cek PC Path
        cooldownLabel = mainGui["主界面"]["技能"]["按键"][pathName]["按钮"]["倒计时"]
        if not cooldownLabel or not cooldownLabel.IsDescendantOf(game) then
            -- Cek Mobile Path jika PC tidak ditemukan
            cooldownLabel = mainGui["主界面"]["技能"]["触摸"][pathName]["按钮"]["倒计时"]
        end
    end)
