...
local TASK_STATUS = {
    ...
    wheelSpun = false,
    arenaCompleted = true,
    guildDungeonCompleted = false,
    bossMechaCompleted = false,
    worldAFKCompleted = false,
}
...
local TeleportRemote = ReplicatedStorage:WaitForChild("\228\186\139\228\187\182"):WaitForChild("\229\133\172\231\148\168"):WaitForChild("\229\156\186\230\153\175"):WaitForChild("\228\188\160\233\128\129")
local isTPHookRunning, tpConnection = true, nil
local function startTeleportHook()
    ...
    for _, conn in ipairs(connections) do
        ...
        tpConnection = TeleportRemote.OnClientEvent:Connect(function(serverPos, isInstant, ...)
            ...
            local newPos = originalPos + ((Vector3.new(0, originalPos.Y, 0) - originalPos).Unit * 50)
            oldFunction((typeof(serverPos) == "CFrame") and (CFrame.new(newPos) * (serverPos - serverPos.Position)) or newPos, isInstant, ...)
        end)
    end
end

-- Fungsi untuk mengambil menit dari teks format "Nama [MM:SS]"
local function dapatkanMenit(teksUtuh)
    -- Pattern "%[(%d+):(%d+)%]" akan mencari angka di dalam [xx:xx]
    local menit, detik = teksUtuh:match("%[(%d+):(%d+)%]")
    
    if menit then
        -- tonumber() digunakan untuk mengubah string "02" menjadi angka (number) 2
        return tonumber(menit), tonumber(detik)
    end
    return nil, nil
end

local cachedTPX, cachedTPY, cachedTPZ = nil, nil, nil
local function teleportHome()
    local success, err = pcall(function()
        if not cachedTPX then
            ...
            local reworld = game:GetService('Workspace'):waitForChild('主場景' .. RespawPointnum):waitForChild('重生点')
            cachedTPX, cachedTPY, cachedTPZ = reworld.Position.X, reworld.Position.Y + 5, reworld.Position.Z
        end
        
        local char = player.Character or player.CharacterAdded:Wait()
        local hrp = char:WaitForChild('HumanoidRootPart', 10)
        if hrp and cachedTPX then
            hrp.CFrame = CFrame.new(cachedTPX, cachedTPY, cachedTPZ)
        end
    end)
    if not success then
        warn("Teleport home failed: " .. tostring(err))
    end
end
local teleporthome = teleportHome

-- ============================================================================
-- AUTO ARENA HELPER FUNCTIONS
-- ============================================================================
local function runAutoArena()
    local success, err = pcall(function()
        ...
        local isAutoSkillActive = playerGui.GUI["\228\184\187\231\149\140\233\157\162"]["\230\138\128\232\131\189"]["\230\140\137\233\148\174"]["\230\138\128\232\131\189\232\135\170\229\138\168"]["\230\140\137\233\146\174"]["\229\134\133\229\156\136\232\131\140\230\153\175"].UIStroke["\229\189\169\232\137\178\230\143\143\232\190\185"].enabled
        if not isAutoSkillActive then
            ReplicatedStorage:WaitForChild("\228\186\139\228\187\182"):WaitForChild("\229\133\172\231\148\168"):WaitForChild("\232\174\190\231\189\174"):WaitForChild("\231\142\169\229\174\182\228\191\174\230\148\185\232\174\190\231\189\174"):FireServer("\232\135\170\229\138\168\230\138\128\232\131\189")
        end

        local myPower = parseNumber(playerGui.GUI:waitForChild('主界面'):waitForChild('主城'):waitForChild('基础功能区'):waitForChild('战斗力'):waitForChild('当前值').Text)
        
        local function getChallengesLeft()
            local text = playerGui.GUI:waitForChild('二级界面'):waitForChild('竞技场'):waitForChild('背景'):waitForChild('右侧界面'):waitForChild('竞技场'):waitForChild('标题'):waitForChild('文本').Text
            return tonumber(text:match('%d+')) or 0
        end

        local challengesLeft = getChallengesLeft()

        local refreshButton = playerGui.GUI["\228\186\140\231\186\167\231\149\140\233\157\162"]["\231\171\158\230\138\128\229\156\186"]["\232\131\140\230\153\175"]["\229\143\179\228\190\167\231\149\140\233\157\162"]["\231\171\158\230\138\128\229\156\186"]["\229\136\183\230\150\176\229\175\185\230\137\139"]["\230\140\137\233\146\174"]["\229\144\141\231\167\176"]
        local priorityLeft = (refreshButton.Text == 'Refresh Opponent') and 4 or 2

        local function tryChallenge()
            if priorityLeft == 2 and refreshButton.Text == 'Refresh Opponent' then
                ReplicatedStorage:WaitForChild("\228\186\139\228\187\182"):WaitForChild("\229\133\172\231\148\168"):WaitForChild("\231\171\158\230\138\128\229\156\186"):WaitForChild("\229\136\183\230\150\176\229\175\185\230\137\139"):FireServer()
                task.wait(3)
            end

            for opponentIndex = 1, 4 do
                challengesLeft = getChallengesLeft()
                if challengesLeft <= 0 then break end
                
                local opponent = playerGui.GUI:waitForChild('二级界面'):waitForChild('竞技场'):waitForChild('背景'):waitForChild('右侧界面'):waitForChild('竞技场'):waitForChild('列表'):waitForChild('对手信息' .. parseNumber(opponentIndex))
                local opponentStatus = opponent:waitForChild('挑战'):waitForChild('按钮'):waitForChild('名称').Text
                local opponentPower = parseNumber(opponent:waitForChild('战斗力区域'):waitForChild('值').Text)

                local priorityChallenge = priorityLeft <= 1 or (myPower > opponentPower and opponentPower < 1e9)
                if opponentStatus == 'Challenge' and priorityChallenge then
                    print('Start fight: ' .. opponentIndex .. ', challengeLeft: ' .. challengesLeft)
                    ReplicatedStorage:WaitForChild("\228\186\139\228\187\182"):WaitForChild("\229\133\172\231\148\168"):WaitForChild("\231\171\158\230\138\128\229\156\186"):WaitForChild("\230\140\145\230\136\152"):FireServer(opponentIndex)
                    task.wait(5)
                    teleporthome()
                    task.wait(5)
                end
            end
            priorityLeft = priorityLeft - 1
        end

        while getChallengesLeft() > 0 do
            tryChallenge()
            task.wait(1)
        end

        print('Challenges finished. Buying Souls...')
        local args = {
            11,
            7
        }
    ...
    if not success then
        warn("Auto Arena Error: " .. tostring(err))
    end
    
    arenaFinished = true
    TASK_STATUS.arenaCompleted = true
end

-- Dungeon Task Wrapper
local function runDungeonTask(taskName, taskKey, whitelistMode, teleportFunc, loops, onComplete)
    local playerName = player.Name
    local isWhitelisted = table.find(whiteList, playerName) ~= nil

    if whitelistMode == "whitelistOnly" then
        if not isWhitelisted then
            warn("Username not allowed to " .. taskName .. " (Not in whitelist)")
            pcall(function()
                task[taskKey] = true
            end)
            TASK_STATUS[taskKey] = true
            return
        end
    elseif whitelistMode == "blacklist" then
        if isWhitelisted then
            warn("Username not allowed to " .. taskName .. " (Is in whitelist)")
            pcall(function()
                task[taskKey] = true
            end)
            TASK_STATUS[taskKey] = true
    ...
    for loop = 1, loops do
        if loops > 1 then
            print("[" .. taskName .. "] Starting run " .. loop .. " of " .. loops)
        else
            print("[" .. taskName .. "] Starting task...")
        end

        -- Teleport to dungeon/stage
        pcall(teleportFunc)

        -- Monitor time and changes
        local dynamicTextLabel = playerGui:WaitForChild("GUI")["\228\184\187\231\149\140\233\157\162"]["\230\136\152\230\150\151"]["\229\133\179\229\141\161\228\191\161\230\129\175"]["\230\150\135\230\156\172"]
        local lastDetik = nil
        local lastChangeTime = tick()

        while true do
            local teksUtuh = dynamicTextLabel.Text
            local menit, detik = dapatkanMenit(teksUtuh)

            if menit and detik then
                if menit >= 2 then
                    print("[" .. taskName .. "] Minute reached 2. Exiting.")
                    break
                end

                if detik ~= lastDetik then
                    lastDetik = detik
                    lastChangeTime = tick()
                elseif tick() - lastChangeTime >= 3 then
                    print("[" .. taskName .. "] Seconds did not change for 3 seconds. Exiting.")
                    break
                end
            else
                lastChangeTime = tick()
...
-- AUTO GUILD DUNGEON
local function runAutoGuildDungeon()
    runDungeonTask("Guild Dungeon", "guildDungeonCompleted", nil, function()
        local args = { 1 }
        game:GetService("ReplicatedStorage"):WaitForChild("\228\186\139\228\187\182"):WaitForChild("\229\133\172\231\148\168"):WaitForChild("\229\133\172\228\188\154"):WaitForChild("\232\191\155\229\133\165\229\133\172\228\188\154\229\137\175\230\156\172"):FireServer(unpack(args))
    end)
end

-- AUTO BOSS MECHA
local function runBossMecha()
    runDungeonTask("Boss Mecha", "bossMechaCompleted", "whitelistOnly", function()
        local args = { 1 }
        game:GetService("ReplicatedStorage"):WaitForChild("\228\186\139\228\187\182"):WaitForChild("\229\133\172\231\148\168"):WaitForChild("\228\184\150\231\149\140boss"):WaitForChild("\232\191\155\229\133\179\229\141\161"):FireServer(unpack(args))
    end)
end

-- AUTO WORLD AFK
local function runWorldAFK()
    runDungeonTask("World AFK", "worldAFKCompleted", "blacklist", function()
        local args = { 1 }
        game:GetService("ReplicatedStorage"):WaitForChild("\228\186\139\228\187\182"):WaitForChild("\229\133\172\231\148\168"):WaitForChild("\229\133\179\229\141\161"):WaitForChild("\232\191\155\229\133\165\228\184\150\231\149\140\229\133\179\229\141\161"):FireServer(unpack(args))
    end, 3, function()
        local args = {
            8,
            Vector2.new(1259.7445068359375, 731.5599975585938)
        }
        game:GetService("ReplicatedStorage"):WaitForChild("\228\186\139\228\187\182"):WaitForChild("\229\133\172\231\148\168"):WaitForChild("\230\175\143\230\151\165\228\187\187\229\138\161"):WaitForChild("\233\162\134\229\143\150\229\165\150\229\138\177"):FireServer(unpack(args))
        task.wait(0.5)
    end)
end
...
local function verifyAllTasksCompleted()
    ...
    for taskName, completed in pairs(TASK_STATUS) do
        if not completed then
            -- We don't mark as failed if it's still being waited for in the main loop
            allComplete = false
            table.insert(failedTasks, taskName)
        end
    end
    
    return allComplete, failedTasks
end
...
local function main()
    ...
    updateStatusLabel(label, "Starting Tasks...")
    startTeleportHook()
    
    -- Spawn Arena, Guild Dungeon, Boss Mecha & WorldAFK (Sequentially to prevent teleport conflict)
    task.spawn(function()
        -- runAutoArena()
        runAutoGuildDungeon()
        runBossMecha()
    ...
    -- Spawn Mail Claim (Background)
    -- task.spawn(function()
    --     claimMail()
    -- end)

    -- Spawn Gem Investment (Background)
    task.spawn(function()
        investGems()
    end)

    -- Spawn Herb Collection (Background)
    task.spawn(function()
        collectHerbs()
    end)

    -- Spawn Guild Tasks (Background)
    task.spawn(function()
        toggleGuildUI(true)
        task.wait(CONFIG.UI_UPDATE_DELAY)
        donateToGuild()
        purchaseHerbs()
        toggleGuildUI(false)
    end)

    -- Spawn Wheel Spin (Background)
    task.spawn(function()
        spinWheel()
    end)

    -- Spawn Webhook (Sequential - Run after verification)
    -- This section is now moved after the while loop

    -- Verification Loop
    while true do
        local allComplete, failed = verifyAllTasksCompleted()
        if allComplete then
            break
        end

        updateStatusLabel(label, "In Progress: " .. #failed .. " tasks")
        print("[Status] Waiting for: " .. table.concat(failed, ", "))
        task.wait(2)
    end

    -- Send Webhook sequentially after all cultivation tasks are complete
    updateStatusLabel(label, "Reporting Stats...")
    sendWebhook()
